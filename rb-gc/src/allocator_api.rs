...
impl<'gc> MetricsAlloc<'gc> {
  #[inline]
  pub fn new(mc: &Mutation<'gc>) -> Self { Self::new_in(mc, Global) }

  /// `MetricsAlloc` is normally branded with the `'gc` branding lifetime to
  /// ensure that it is not placed in the wrong arena or used outside of the
  /// enclosing arena.
  ///
  /// This is actually completely artificial and only used as a lint:
  /// `gc_arena::metrics::Metrics` has no lifetime at all. Therefore, we can
  /// safely provide a method that returns a `MetricsAlloc` with an arbitrary
  /// lifetime.
  ///
  /// NOTE: Use `MetricsAlloc::new` if at all possible, because it is harder to
  /// misuse.
  #[inline]
  pub fn from_metrics(metrics: Metrics) -> Self { Self::from_metrics_in(metrics, Global) }
}

impl<'gc, A> MetricsAlloc<'gc, A> {
  #[inline]
  pub fn new_in(mc: &Mutation<'gc>, allocator: A) -> Self {
    Self { metrics: mc.metrics().clone(), allocator, _marker: PhantomData }
  }

  #[inline]
  pub fn from_metrics_in(metrics: Metrics, allocator: A) -> Self {
    Self { metrics, allocator, _marker: PhantomData }
  }
}
...
unsafe impl<'gc, A: Allocator> Allocator for MetricsAlloc<'gc, A> {
  ...
  unsafe fn grow(
    ...
    self.metrics.mark_external_allocation(new_layout.size() - old_layout.size());
    Ok(ptr)
  }
  ...
  unsafe fn shrink(
    ...
    self.metrics.mark_external_deallocation(old_layout.size() - new_layout.size());
    Ok(ptr)
  }
}
...
unsafe impl<T: Collect, A: Collect + Allocator> Collect for vec::Vec<T, A> {
  #[inline]
  fn needs_trace() -> bool { T::needs_trace() || A::needs_trace() }

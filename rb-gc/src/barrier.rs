...
impl<T: ?Sized> Deref for Write<T> {
  ...
  #[inline(always)]
  fn deref(&self) -> &Self::Target { &self.__inner }
}

impl<T: ?Sized> DerefMut for Write<T> {
  #[inline(always)]
  fn deref_mut(&mut self) -> &mut Self::Target { &mut self.__inner }
}
...
impl<T: ?Sized> Write<T> {
  ...
  /// the interior mutability enabled by this wrapper, unless [`Gc::write`] is
  /// invoked manually on the parent [`Gc`] pointer during the current arena
  /// callback.
  #[inline(always)]
...
pub trait Unlock {
  ...
  /// Provides unsafe access to the unlocked type, *without* triggering a write
  /// barrier.
  ///
  /// # Safety
  ///
  /// In order to maintain the invariants of the garbage collector, no new `Gc`
  /// pointers may be adopted by as a result of the interior mutability
...
macro_rules! __field {
  ($value:expr, $type:path, $field:ident) => {
    ...
    // they may access nested `Gc` pointers, which would violate the write barrier
    // invariant. This is guaranteed as follows:
    ...
    match $value {
      $crate::barrier::Write { __inner: $type { ref $field, .. }, .. } => unsafe {

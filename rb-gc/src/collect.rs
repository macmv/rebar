...
/// followed:
///
///   1. `Collect::trace` *must* trace over *every* `Gc` pointer held inside
///      this type, and cannot fail.
///   2. Held `Gc` pointers must not be accessed inside `Drop::drop` since
///      during drop any such pointer may be dangling.
///   3. Internal mutability *must* not be used to adopt new `Gc` pointers
///      without calling `Gc::write` during the same arena mutation.
///
...
/// store `Gc` pointers inside them, so the write barrier requirement cannot be
...
pub unsafe trait Collect<C> {
  ...
  /// *Must* call `Collect::trace` on all held `Gc` pointers. If this type holds

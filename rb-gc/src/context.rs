...
impl<'gc> Mutation<'gc> {
  ...
  /// method is one of the safe ways for the value in the `parent` pointer to
  /// use internal mutability to adopt the `child` pointer without
  ...
  /// colored black, AND the `child` is colored white, then immediately change
  /// the `child` to gray and enqueue it for tracing.
  ///
  /// This operation is known as a "forwards write barrier". Calling this method
  /// is one of the safe ways for the value in the `parent` pointer to use
  /// internal mutability to adopt the `child` pointer without invalidating
  /// the color invariant.
  ///
  /// If the `parent` parameter is given, then calling this method ensures that
  /// the `parent` pointer may safely adopt the `child` pointer. If no
  /// `parent` is given, then calling this method is more general, and it
  ...
  #[inline]
  pub(crate) fn upgrade(&self, gc_box: GcBox) -> bool { self.context.upgrade(gc_box) }
}
...
/// Handle value given by arena callbacks during garbage collection, which must
/// be passed through `Collect::trace` implementations.
#[repr(transparent)]
...
impl Collection {
  ...
  #[inline]
  pub(crate) fn trace(&self, gc_box: GcBox) { self.context.trace(gc_box) }

  #[inline]
  pub(crate) fn trace_weak(&self, gc_box: GcBox) { self.context.trace_weak(gc_box) }
}
...
impl Context {
  ...
  #[inline]
  pub(crate) fn metrics(&self) -> &Metrics { &self.metrics }
  ...
  #[inline]
  pub(crate) fn phase(&self) -> Phase { self.phase.get() }
  ...
  // Do some collection work until either the debt goes down below the target
  // amount or we have finished the gc sweep phase. The unit of "work" here is a
  // byte count of objects either turned black or freed, so to completely
  // collect a heap with 1000 bytes of objects should take 1000 units of work,
  // whatever percentage of them are live or not.
  //
        ...
        Phase::Mark => {
          ...
          if let Some(gc_box) = next_gray {
            ...
            impl<'a> Drop for DropGuard<'a> {
              fn drop(&mut self) { self.cx.gray_again.borrow_mut().push(self.gc_box); }
            }
  ...
  fn allocate<T: Collect>(&self, t: T) -> NonNull<GcBoxInner<T>> {
    ...
    // Make the generated code easier to optimize into `T` being constructed in
    // place or at the very least only memcpy'd once.
  ...
  fn backward_barrier(&self, parent: GcBox, child: Option<GcBox>) {
    ...
    // white object to it and invalidate the invariant that black objects may
    // not point to white objects. Turn the black parent object gray to prevent
    // this.
    //
    // NOTE: This also adds the pointer to the gray_again queue even if
    // `header.needs_trace()` is false, but this is not harmful (just wasteful).
    // There's no reason to call a barrier on a pointer that can't adopt other
    // pointers, so we skip the check.
    if self.phase.get() == Phase::Mark
  ...
  fn forward_barrier(&self, parent: Option<GcBox>, child: GcBox) {
    // During the marking phase, if we are mutating a black object, we may add a
    // white object to it and invalidate the invariant that black objects may
    // not point to white objects. Immediately trace the child white object to
    // turn it gray (or black) to prevent this.
    if self.phase.get() == Phase::Mark
      && parent.map(|p| p.header().color() == GcColor::Black).unwrap_or(true)
    {
      self.trace(child);
    }
  }
  ...
  fn upgrade(&self, gc_box: GcBox) -> bool {
    ...
    //   `WhiteWeak` is only  set when a white `GcWeak/ GcWeakCell` is traced. A
    //   `GcWeak/GcWeakCell` must be created from an existing `Gc/ GcCell` via
    //   `downgrade()`, so `WhiteWeak` means that a `GcWeak` / `GcWeakCell` existed
    ...
    // `GcWeak/GcWeakCell`. Since it is   not `WhiteWeak` there cannot have been
    // any `GcWeak/GcWeakCell`s during the   last `Phase::Mark`, so the weak
    // pointer must have been created during this   `Phase::Sweep`. This is only
    // possible if the underlying allocation was freshly-created
...
unsafe fn free_gc_box<'gc>(mut gc_box: GcBox) {
  if gc_box.header().is_live() {
    // If the alive flag is set, that means we haven't dropped the inner value of
    // this object,

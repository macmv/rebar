...
impl DynamicRootSet {
  ...
  /// Gets immutable access to the given root, or returns an error if the handle
  /// doesn't belong to this root set.
  #[inline]
  ...
  pub fn contains<R: for<'r> Rootable<'r>>(&self, root: &DynamicRoot<R>) -> bool {
    ...
    // true and surely MUST continue to be true, but is possibly under-specified
    // in the stdlib. We are assuming that if the `Weak` pointer held in the
...
impl<R: for<'gc> Rootable<'gc>> Clone for DynamicRoot<R> {
  fn clone(&self) -> Self {
    ...
    Self { ptr: self.ptr, slots: self.slots.clone(), index: self.index }
  }
}
...
impl<R: for<'gc> Rootable<'gc>> DynamicRoot<R> {
  ...
  /// calling the unsafe [`Gc::from_ptr`], but this is incredibly dangerous!
  ///
  /// First, if the [`DynamicRootSet`] in which the `DynamicRoot` was stashed
  /// has been collected, then either the returned pointer or other transitive
  /// `Gc` pointers objects may be dangling. The parent `DynamicRootSet`
  /// *must* still be uncollected in order to do this soundly.
  ///
  /// Second, the `'gc` lifetime returned here is unbound, so it is meaningless
  /// and can allow improper mixing of objects across arenas. The returned
  /// `'gc` lifetime must be bound to only the arena that holds the parent
  /// `DynamicRootSet`.
  #[inline]
...
impl fmt::Display for MismatchedRootSet {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { f.write_str("mismatched root set") }
}
  ...
  fn add(&mut self, p: Gc<()>) -> Index {
    ...
    if self.next_free != NULL_INDEX {
      ...
      *slot = Slot::Occupied { root: p, ref_count: 0 };
      ...
      self.slots.push(Slot::Occupied { root: p, ref_count: 0 });
      ...
      if new_capacity > old_capacity {
        self
          .metrics
          .mark_external_allocation((new_capacity - old_capacity) * mem::size_of::<Slot>());
      }
  ...
  fn inc(&mut self, idx: Index) {
    match &mut self.slots[idx] {
      Slot::Occupied { ref_count, .. } => {
        *ref_count = ref_count.checked_add(1).expect("DynamicRoot refcount overflow!");
      }

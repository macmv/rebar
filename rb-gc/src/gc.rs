...
impl<'gc, T: Collect + 'gc> Gc<'gc, T> {
  ...
  pub fn new(mc: &Mutation<'gc>, t: T) -> Gc<'gc, T> {
    Gc { ptr: mc.allocate(t), _invariant: PhantomData }
  }
}
...
impl<'gc, T: ?Sized + 'gc> Gc<'gc, T> {
  ...
  pub unsafe fn cast<U: 'gc>(this: Gc<'gc, T>) -> Gc<'gc, U> {
    Gc { ptr: NonNull::cast(this.ptr), _invariant: PhantomData }
  }
  ...
  #[inline]
  pub fn erase(this: Gc<'gc, T>) -> Gc<'gc, ()> { unsafe { Gc::cast(this) } }
  ...
  pub unsafe fn from_ptr(ptr: *const T) -> Gc<'gc, T> {
    ...
    Gc { ptr: NonNull::new_unchecked(ptr), _invariant: PhantomData }
  }
}
...
impl<'gc, T: ?Sized + 'gc> Gc<'gc, T> {
  ...
  #[inline]
  pub fn downgrade(this: Gc<'gc, T>) -> GcWeak<'gc, T> { GcWeak { inner: this } }

...
impl GcBox {
  ...
  #[inline(always)]
  pub(crate) fn header(&self) -> &GcBoxHeader { unsafe { &self.0.as_ref().header } }
  ...
  #[inline(always)]
  pub(crate) unsafe fn drop_in_place(&mut self) { (self.header().vtable().drop_value)(*self) }
...
impl GcBoxHeader {
  ...
  #[inline(always)]
  pub(crate) fn next(&self) -> Option<GcBox> { self.next.get() }

  /// Sets the next element in the global linked list of allocated objects.
  #[inline(always)]
  pub(crate) fn set_next(&self, next: Option<GcBox>) { self.next.set(next) }

  /// Returns the (shallow) size occupied by this box in memory.
  #[inline(always)]
  pub(crate) fn size_of_box(&self) -> usize { self.vtable().box_layout.size() }
  ...
  /// pointers, we set `alive` to false. When there are no more weak pointers
  /// remaining, we will deallocate the `GcBox`, but skip dropping the `dyn
  /// Collect` value (since we've already done it).
  #[inline]
...
pub(crate) enum GcColor {
  /// An object that has not yet been reached by tracing (if we're in a tracing
  /// phase).
  ///
  ...
  ///
  /// These objects may drop their contents during `Phase::Sweep`, but must stay
  /// allocated so that weak references can check the alive status.
  WhiteWeak,
  /// An object reachable from a Black object, but that has not yet been traced
  /// using `Collect::trace`. We also mark black objects as gray during
  /// `Phase::Mark` in response to a write barrier, so that we re-trace and
  /// find any objects newly reachable from the mutated object.
  Gray,
  /// An object that was reached during tracing. It will not be freed during
  /// `Phase::Sweep`. At the end of `Phase::Sweep`, all black objects will be
  /// reset to white.
  Black,
}
